...
namespace AsterNET.Manager
{
    ...
    public class ManagerReader
    {
        ...
        private bool processingNewCommandResult;
        ...
        internal void Reinitialize()
        {
            ...
            processingNewCommandResult = false;
            ...
            while (true)
            {
                try
                {
                    while (!die)
                    {
                        ...
                        else if (processingNewCommandResult)
                        {
                            string lineLower = line.ToLower(Helper.CultureInfo);
                        ...
                        if (!string.IsNullOrEmpty(line))
                        {
                            ...
                            else if (line.Trim().ToLower(Helper.CultureInfo) == "message: command output follows")
                            {
                                // Switch to wait "non output" response
                                processingNewCommandResult = true;
                                //packet.Clear();
                                commandList.Clear();
                                Helper.AddKeyValue(packet, line);
                                continue;
